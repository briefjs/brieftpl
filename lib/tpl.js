...
export function renderTag(textNodes) {
...
export function tagged(nodeName) {
  var selfClosing = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
  return function () {
    if (Array.isArray(arguments.length <= 0 ? undefined : arguments[0])) {
      // no attributes
      return adapter.createElement.apply(adapter, [nodeName, {}].concat(_toConsumableArray(renderTag.apply(void 0, arguments))));
    }

    var attrs = arguments.length <= 0 ? undefined : arguments[0];
    return function () {
      return adapter.createElement.apply(adapter, [nodeName, attrs].concat(_toConsumableArray(renderTag.apply(void 0, arguments))));
    };
  };
}